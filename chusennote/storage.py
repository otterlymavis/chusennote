"""Persistence, discovery pipeline, and watch orchestration for chusennote.

Holds the SQLite schema/migrations, all watch/event/source/round CRUD, the
keyword->blocks discovery pipeline, batch watch runs, alert lifecycle, and the
calendar/ICS + API read models. The discovery pipeline and persistence are
mutually recursive (a batch run builds blocks, building blocks for a tracked
event reads its manual sources), so they live in one module.
"""
